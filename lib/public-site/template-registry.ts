...
export function isExecutableSiteTemplate(key?: string | null) { const item = getTemplateCatalogRecord(key); return Boolean(item?.capabilities.editorSupported && item.capabilities.previewRenderable && item.capabilities.publicRenderable); }
