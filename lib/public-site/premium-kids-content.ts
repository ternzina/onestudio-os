...
export function addPremiumKidsBlock(content: PremiumKidsContent, type: PremiumKidsBlockType, id: string, mediaPosition?: PublicSiteMediaPosition, afterBlockId?: string) { const definition = getPremiumKidsBlockDefinition(type); if (!definition.capabilities.add || content.blocks.some((block) => block.id === id)) return content; const blocks = [...content.blocks]; const selectedIndex = afterBlockId ? blocks.findIndex((block) => block.id === afterBlockId) : -1; const insertionIndex = selectedIndex >= 0 ? Math.min(Math.max(selectedIndex + 1, 2), blocks.length - 1) : blocks.length - 1; blocks.splice(insertionIndex, 0, createPremiumKidsDefaultBlock(type, DEFAULT_PREMIUM_KIDS_CONTENT, id, mediaPosition)); return replacePremiumKidsBlocks(content, blocks); }
