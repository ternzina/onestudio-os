...
export const GUIDE_PUBLISHER_PLATFORM_ORIGIN = "https://onestudioos.com";
const GUIDE_SLUG_PATTERN = /^[a-z0-9]+(?:-[a-z0-9]+)*$/;

export function guidePublicationIndexNowUrls(
  slugs,
  origin = GUIDE_PUBLISHER_PLATFORM_ORIGIN,
) {
  const base = new URL(origin);
  if (base.protocol !== "https:" || base.hostname !== "onestudioos.com") {
    throw new Error("Guide IndexNow origin must be https://onestudioos.com");
  }
  const urls = [new URL("/", base).toString(), new URL("/guides", base).toString()];
  for (const rawSlug of slugs) {
    const slug = String(rawSlug ?? "").trim();
    if (!GUIDE_SLUG_PATTERN.test(slug)) throw new Error(`Invalid Guide slug for IndexNow: ${slug}`);
    urls.push(new URL(`/guides/${slug}`, base).toString());
  }
  return [...new Set(urls)];
}
