-- OneStudio OS Multi-workspace + Auto-publish 1.0
-- Allows an account to own multiple isolated sites. New demo locales are published immediately.
...
begin
  ...
  -- The workspace just created becomes the active/default context. Older
  -- workspaces remain available and isolated in the account dashboard.
  ...
  set draft_content = public.public_site_locales.draft_content || excluded.draft_content,
      updated_at = now();

  -- A newly created demo must open immediately. The editor still keeps draft
  -- and published snapshots separate for all later customer changes.
  update public.public_site_locales
  set published_content = draft_content,
      published_at = now(),
      updated_at = now()
  where business_id = v_business_id
    and locale = any(v_locales);
