-- OneStudio OS Site Composition 2.4
-- Adds tenant-safe split media/text sections and two/three-column sections.
-- Existing draft and publication functions pick up the new normalized shape
-- without weakening workspace permissions or allowing arbitrary file content.
...
begin
  ...
  loop
    ...
    v_media_type := case
      when v_item->>'media_type' = 'video' then 'video'
      else 'image'
    end;
...
is 'Normalizes tenant-safe site blocks including split media/text and two/three-column layouts.';
