-- OneStudio OS: VELORA customer creation release 1.0
-- Adds the released VELORA package to the canonical creation allow-list and
-- preserves a separate seed for every requested site locale.
...
begin
  ...
  if v_template not in ('standard', 'gloss-nail-studio', 'premium-kids-center', 'premium-studio', 'velora-event-venue') then raise exception 'template_key_invalid' using errcode = '22023'; end if;
  ...
  v_legacy_demo := case v_template
    when 'premium-studio' then 'frame-house'
    when 'premium-kids-center' then 'little-orbit'
    else 'lumiere'
  end;
...
comment on function public.create_template_workspace(jsonb) is
  'Canonical idempotent and atomic customer creation contract for one unpublished workspace, business setup, offer, resource, availability, modules and locale-specific template seeds.';
