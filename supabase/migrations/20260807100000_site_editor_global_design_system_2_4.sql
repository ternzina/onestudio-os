...
--
-- Adds a sparse, template-safe design layer for typography, buttons and cards.
-- Empty / missing settings intentionally preserve the original template design.

create or replace function public.normalize_public_site_design_system(
  p_design jsonb
)
returns jsonb
language plpgsql
...
begin
  ...
  v_group := p_design->'typography';
  if jsonb_typeof(v_group) = 'object' then
    v_normalized := '{}'::jsonb;

    if v_group ? 'body_font' then
      v_value := lower(trim(coalesce(v_group->>'body_font', '')));
      if v_value in ('system', 'humanist', 'editorial') then
        v_normalized := v_normalized || jsonb_build_object('body_font', v_value);
      end if;
    end if;

    if v_group ? 'heading_font' then
      v_value := lower(trim(coalesce(v_group->>'heading_font', '')));
      if v_value in ('system', 'humanist', 'editorial') then
        v_normalized := v_normalized || jsonb_build_object('heading_font', v_value);
      end if;
    end if;

    if v_group ? 'heading_weight' then
      v_value := lower(trim(coalesce(v_group->>'heading_weight', '')));
      if v_value in ('regular', 'medium', 'semibold', 'bold') then
        v_normalized := v_normalized || jsonb_build_object('heading_weight', v_value);
      end if;
    end if;

    if v_group ? 'heading_tracking' then
      v_value := lower(trim(coalesce(v_group->>'heading_tracking', '')));
      if v_value in ('tight', 'normal', 'wide') then
        v_normalized := v_normalized || jsonb_build_object('heading_tracking', v_value);
      end if;
    end if;

    if v_normalized <> '{}'::jsonb then
      v_result := v_result || jsonb_build_object('typography', v_normalized);
    end if;
  end if;

  v_group := p_design->'buttons';
  if jsonb_typeof(v_group) = 'object' then
    v_normalized := '{}'::jsonb;

    if v_group ? 'radius' then
      v_value := lower(trim(coalesce(v_group->>'radius', '')));
      if v_value in ('square', 'soft', 'rounded', 'pill') then
        v_normalized := v_normalized || jsonb_build_object('radius', v_value);
      end if;
    end if;

    if v_group ? 'shadow' then
      v_value := lower(trim(coalesce(v_group->>'shadow', '')));
      if v_value in ('none', 'soft', 'strong') then
        v_normalized := v_normalized || jsonb_build_object('shadow', v_value);
      end if;
    end if;

    if v_normalized <> '{}'::jsonb then
      v_result := v_result || jsonb_build_object('buttons', v_normalized);
    end if;
  end if;

  v_group := p_design->'cards';
  if jsonb_typeof(v_group) = 'object' then
    v_normalized := '{}'::jsonb;

    if v_group ? 'radius' then
      v_value := lower(trim(coalesce(v_group->>'radius', '')));
      if v_value in ('square', 'soft', 'rounded') then
        v_normalized := v_normalized || jsonb_build_object('radius', v_value);
      end if;
    end if;

    if v_group ? 'border' then
      v_value := lower(trim(coalesce(v_group->>'border', '')));
      if v_value in ('none', 'subtle', 'strong') then
        v_normalized := v_normalized || jsonb_build_object('border', v_value);
      end if;
    end if;
    ...
    if v_normalized <> '{}'::jsonb then
      v_result := v_result || jsonb_build_object('cards', v_normalized);
    end if;
  end if;
...
begin
  ...
  v_design_system := public.normalize_public_site_design_system(
    case
      when jsonb_typeof(p_content->'design_system') = 'object'
        then p_content->'design_system'
      when jsonb_typeof(v_previous->'design_system') = 'object'
        then v_previous->'design_system'
      else '{}'::jsonb
    end
  );
...
comment on function public.save_public_site_draft(uuid, text, jsonb, boolean) is
  'Delegates validated fields to save_public_site_draft_v22 and preserves template, palette, layout order, section colors, built-in section settings and the sparse Global Design System 2.4.';

comment on function public.normalize_public_site_design_system(jsonb) is
  'Validates sparse Global Design System 2.4 typography, button and card tokens; template defaults are represented by absent tokens.';
