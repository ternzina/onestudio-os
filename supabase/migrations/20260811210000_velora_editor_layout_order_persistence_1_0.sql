-- OneStudio OS
-- VELORA editor layout-order persistence 1.0
--
-- Preserve the exact native composition submitted by the VELORA editor. The
-- wrapper is deliberately idempotent because this release can be applied once
-- through SQL Editor and later recorded through the normal migration pipeline.
...
as $$
  ...
  from (
    ...
    where
      ...
      or (
        lower(trim(coalesce(p_template_id, ''))) = 'velora-event-venue'
        and item ~ '^native:velora-event-venue:(hero|facts|venues|formats|transformation|story|packages|included|catering|decor|coordinator|reviews|gallery|planner|faq|availability|footer)$'
      )
    group by item
...
begin
  if to_regprocedure(
    'public.save_public_site_draft_v_velora_layout_1_0(uuid,text,jsonb,boolean)'
  ) is null then
    alter function public.save_public_site_draft(uuid, text, jsonb, boolean)
      rename to save_public_site_draft_v_velora_layout_1_0;
  end if;
end;
$$;
...
begin
  ...
  v_saved := public.save_public_site_draft_v_velora_layout_1_0(
    p_business_id,
    p_locale,
    p_content,
    p_make_primary
  ...
  v_layout := public.normalize_public_site_layout_order_v3(
    v_layout_source,
    v_template_id
  );
...
revoke all on function public.save_public_site_draft_v_velora_layout_1_0(
...
comment on function public.normalize_public_site_layout_order_v3(jsonb, text) is
  'Validates bounded public-site layout order and allows exact native section ids for the matching NOIR or VELORA template.';

comment on function public.save_public_site_draft(uuid, text, jsonb, boolean) is
  'Site Editor save pipeline with VELORA native layout-order round-trip preservation.';
