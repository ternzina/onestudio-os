...
alter table public.site_monetization_settings
...
begin
  ...
  select jsonb_build_object(
    ...
    'adsense_enabled', coalesce(settings.adsense_enabled, false),
...
create or replace function public.save_site_monetization_settings_v2(
  p_business_id uuid,
...
begin
  ...
  if coalesce(p_adsense_enabled, false)
     and normalized_publisher_id = '' then
    raise exception using
      errcode = '22023',
      message = 'adsense_publisher_id_required';
  end if;
  ...
  values (
    ...
    coalesce(p_adsense_enabled, false)
  )
...
create or replace function public.resolve_public_site_adsense(
  p_domain text
)
returns jsonb
language sql
...
as $$
  ...
  where (
    ...
    and settings.adsense_enabled = true
    and settings.adsense_publisher_id ~ '^ca-pub-[0-9]{16}$'
  limit 1;
$$;

revoke all on function public.save_site_monetization_settings_v2(uuid, text, boolean, text, boolean)
  from public, anon, authenticated;
grant execute on function public.save_site_monetization_settings_v2(uuid, text, boolean, text, boolean)
  to authenticated;

revoke all on function public.resolve_public_site_adsense(text)
  from public, anon, authenticated;
grant execute on function public.resolve_public_site_adsense(text)
  to anon, authenticated;

comment on function public.resolve_public_site_adsense(text) is
  'Returns the AdSense publisher configuration only for an active verified custom domain and a published active site.';
