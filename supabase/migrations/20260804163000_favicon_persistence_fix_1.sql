-- OneStudio OS
-- Favicon Persistence Fix 1.0.
--
-- Preserves favicon_url together with the existing visual-editor state.
...
begin
  ...
  v_favicon_url := public.normalize_public_site_media_url(
    case
      when p_content ? 'favicon_url'
        then p_content->>'favicon_url'
  ...
  v_saved := jsonb_set(
    v_saved,
    '{favicon_url}',
    coalesce(to_jsonb(v_favicon_url), 'null'::jsonb),
    true
  );
...
comment on function public.save_public_site_draft(
  ...
  'Delegates validated fields to save_public_site_draft_v22 and preserves favicon, template, palette, layout order, mobile title settings and normalized section colors.';
