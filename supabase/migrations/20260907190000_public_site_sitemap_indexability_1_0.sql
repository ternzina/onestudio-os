...
-- PostgreSQL cannot change a function's OUT-column row type in place. The
-- published locale registry is the canonical source for sitemap visibility,
-- so expose its locale-level no-index setting with each SEO path.
drop function if exists public.list_public_site_seo_paths(text);

create function public.list_public_site_seo_paths(
...
comment on function public.list_public_site_seo_paths(text) is
  'Lists published locale paths, locale-level sitemap indexability, and active custom domains for host-aware robots and sitemap generation.';
