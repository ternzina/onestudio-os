-- OneStudio OS
-- Services presentation foundation 1.0.
-- Keeps catalog data canonical while adding draft/publish presentation settings
-- and stable per-service image assignment by service slug.

create or replace function public.normalize_public_site_service_images(p_value jsonb)
returns jsonb
language sql
immutable
set search_path = public
as $$
  select coalesce(jsonb_object_agg(item.key, item.normalized_url), '{}'::jsonb)
  from (
    select
      entry.key,
      public.normalize_public_site_media_url(entry.value) as normalized_url
    from jsonb_each_text(
      case when jsonb_typeof(p_value) = 'object' then p_value else '{}'::jsonb end
    ) entry
    where entry.key ~ '^[a-z0-9]+(?:-[a-z0-9]+)*$'
    order by entry.key
    limit 100
  ) item;
$$;
...
revoke all on function public.normalize_public_site_service_images(jsonb)
from public, anon, authenticated;

grant execute on function public.normalize_public_site_service_images(jsonb)
to service_role;
...
comment on function public.normalize_public_site_service_images(jsonb) is
  'Normalizes a service-slug to image-URL map for public-site service cards.';

comment on function public.get_public_site_editor(uuid) is
  'Returns editable public-site content and active public catalog services for the visual builder.';
