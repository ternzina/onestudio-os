-- OneStudio OS Self-service Workspace 1.0
-- Creates an isolated tenant from a saved public configurator selection.

alter table public.business_launch_profiles
  add column if not exists demo_slug text,
  add column if not exists tagline text not null default '',
  add column if not exists palette_index integer not null default 0,
...
begin
  ...
  select coalesce(array_agg(distinct locale order by locale), array['en']::text[])
  into v_locales
  from jsonb_array_elements_text(coalesce(p_configuration -> 'locales', '["en"]'::jsonb)) item(locale)
  where locale in ('ru', 'en', 'uk', 'pl');
  if cardinality(v_locales) = 0 then v_locales := array['en']::text[]; end if;
  v_default_locale := v_locales[1];

  select coalesce(array_agg(distinct module_key order by module_key), array['core']::text[])
  into v_modules
  from jsonb_array_elements_text(coalesce(p_configuration -> 'enabled_modules', '["core"]'::jsonb)) item(module_key)
  where module_key in (
  ...
  v_modules := array(
    ...
    from unnest(
      ...
      || case when 'portfolio' = any(v_modules) then array['media']::text[] else '{}'::text[] end
      || case when 'notifications' = any(v_modules) then array['payments']::text[] else '{}'::text[] end
      || case when 'documents' = any(v_modules) then array['payments', 'notifications']::text[] else '{}'::text[] end
    ) key
    order by key
  );
  ...
  if v_slug = '' then v_slug := v_demo_slug; end if;
  v_slug := left(v_slug, 42) || '-' || substr(replace(v_business_id::text, '-', ''), 1, 8);

  insert into public.businesses (
    id, slug, name, timezone, default_locale, default_currency, status
  ) values (
    v_business_id, v_slug, v_name, 'Europe/Kyiv', v_default_locale, v_currency, 'active'
  );
  ...
  set enabled = module_key = any(v_modules),
      version = case when module_key = any(v_modules) then '1.0.0' else version end,
      updated_at = now()
