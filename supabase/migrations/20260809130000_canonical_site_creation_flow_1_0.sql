-- OneStudio OS Canonical Site Creation Flow 1.0
-- Forward replacement for the locally-applied Template Catalog Recovery RPC.
-- This migration is intentionally safe for both an already-migrated database and a fresh chain.
...
begin
  ...
  set draft_content = v_seed, published_content = null, published_at = null, updated_at = now()
...
comment on function public.create_template_workspace(jsonb) is
  'Canonical idempotent and atomic customer creation contract for one unpublished workspace, business setup, offer, resource, availability, modules and template seed.';
