...
--
-- Adds validated layout, spacing, height and reveal-animation controls
-- to custom blocks on home and custom public pages.
...
declare
  ...
  v_content_width text;
  v_padding_top text;
  v_padding_bottom text;
  v_section_height text;
  v_media_height text;
  v_animation text;
  v_animate_on_mobile boolean;
...
begin
  ...
  loop
    ...
    v_media_height := case
      when v_item->>'media_height' in ('auto', 'compact', 'medium', 'tall')
        then v_item->>'media_height'
      else 'auto'
    end;
    ...
    v_result := v_result || jsonb_build_array(
      jsonb_build_object(
        ...
        'content_width', v_content_width,
        'padding_top', v_padding_top,
        'padding_bottom', v_padding_bottom,
        'section_height', v_section_height,
        'media_height', v_media_height,
        'animation', v_animation,
        'animate_on_mobile', v_animate_on_mobile
      )
    );
...
comment on function public.normalize_public_site_custom_blocks(jsonb) is
  'Normalizes public custom blocks including Site Editor Visual Controls 2.2 layout, spacing, height and animation settings.';
