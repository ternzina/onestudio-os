...
function VisualBuilder({
  ...
  const missingGuideCategories = Array.from(
    missingGuides.reduce((groups, guide) => {
      const category =
        (guide as typeof guide & { category?: string }).category ??
        "Loan Basics";
  ...
  function repairCashPathGuideRichTextInDraft() {
    onReplaceDraft(
      repairCashPathGuideRichText(draft),
      "cashpath:guide-richtext-repair",
    );
    setPageLibraryOpen(false);
  }
              ...
              <>
                ...
                {missingGuides.length ? (
                  ...
                  >
                    ...
                    <h3 className="mt-2 text-xl font-semibold">
                      SYNC TO DRAFT · {missingGuides.length}{" "}
                      {missingGuides.length === 1 ? "guide" : "guides"}
                    </h3>
                    ...
                    <details
                      className="mt-3 text-sm text-[#182b29]/70"
                    ...
                    >
                      ...
                      <ul className="mt-2 max-h-64 space-y-1 overflow-auto">
                        {missingGuideCategories.flatMap(([category, guides]) =>
                          guides.map((guide) => (
                            <li key={guide.slug}>
                              {guide.title} · {guide.slug} · {category}
                            </li>
                          )),
                        )}
                      </ul>
                    ...
                    <p className="mt-2 text-sm text-[#182b29]/70">
                      Adds only these missing guides to the draft. Existing
                      pages stay unchanged; this does not publish the site.
                    </p>
                  </button>
                ) : null}
                {needsCashPathGuideRepair ? (
                  <button
                    type="button"
                    disabled={!canConfigure}
                    onClick={repairCashPathGuideRichTextInDraft}
                    className="mt-4 w-full rounded-[24px] border border-[#167a6a]/25 bg-[#e8f5f1] p-5 text-left text-[#182b29] disabled:opacity-50"
                  >
                    <p className="text-[10px] font-semibold uppercase tracking-[0.2em] text-[#167a6a]">
                      CashPath
                    ...
                    <h3 className="mt-2 text-xl font-semibold">
                      Исправить повреждённый текст SEO-гайда
                    </h3>
                    <p className="mt-2 text-sm text-[#182b29]/70">
                      Восстановит только повреждённые rich-text поля
                      зарегистрированных SEO-гайдов. Остальные тексты, страницы
                      и настройки не изменятся.
                    </p>
                  </button>
                ) : null}
