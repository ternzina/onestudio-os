...
export default function AdminSitePage() {
  ...
  async function selectTemplateKey(templateKey: string) {
    ...
    replaceDraft(contentToSave, "site-template");
    ...
    if (saveError) {
      ...
      if (savedContent.template_id !== templateKey) {
        draftRef.current = current;
        setDraft(current);
        setTemplateSavedKey(previousSavedTemplateKey);
    ...
    >
      ...
      <div data-admin-site-wrapper className="mx-auto max-w-7xl">
        ...
        <OneStudioDesignDialog
          ...
          activeTemplateKey={draft.template_id ?? "standard"}
          activeDesigns={getActiveEditorDesigns()}
          businessSlug={editor.business.slug}
          locale={selectedLocale}
          canConfigure={canConfigure}
          saving={saving}
          savingTemplateKey={templateSavingKey}
          savedTemplateKey={templateSavedKey}
          onSelectDesign={selectTemplateKey}
...
function VisualBuilder({
  ...
  function addCustomPage() {
    const page = createOneStudioPage(draft, () => `custom-${Date.now()}`);
  ...
  const missingGuideCategories = Array.from(missingGuides.reduce((groups, guide) => {
    const category = (guide as typeof guide & { category?: string }).category ?? "Loan Basics";
    groups.set(category, [...(groups.get(category) ?? []), guide]);
    return groups;
  }, new Map<string, typeof missingGuides>()));
  ...
  function duplicateCustomBlock(targetBlock = selectedCustomBlock) {
    ...
    if (activePage) {
      const blocks = [...(activePage.blocks ?? [])];
      const index = blocks.findIndex((block) => block.id === targetBlock.id);
  ...
  function removeCustomBlock(targetBlock = selectedCustomBlock) {
    if (!targetBlock) return;
    if (activePage) {
      updatePage(
        "blocks",
      ...
      const order = layoutOrder.filter(
        (item) => item !== customBlockLayoutId(targetBlock.id),
      );
      onReplaceDraft({
        ...draft,
        ...
        >
          <div className="max-h-[calc(100dvh-2rem)] w-full max-w-2xl overflow-auto rounded-[30px] bg-[#f8f7f3] p-6 shadow-[0_35px_120px_rgba(0,0,0,0.4)] sm:p-8">
            ...
            {draft.template_id === "cashpath" ? (
              <>
                <button type="button" disabled={!canConfigure || missingCashPathSystemPages.length === 0} onClick={addMissingCashPathPages} className="mt-4 w-full rounded-[24px] border border-[#182b29]/15 bg-[#182b29] p-5 text-left text-[#f7f5ef] disabled:opacity-50">
                  <p className="text-[10px] font-semibold uppercase tracking-[0.2em] text-[#c8ef22]">CashPath system pages</p>
                  <h3 className="mt-2 text-xl font-semibold">{missingCashPathSystemPages.length ? "Add missing CashPath pages" : "CashPath system pages are complete"}</h3>
                  <p className="mt-2 text-sm text-white/70">{missingCashPathSystemPages.length ? `Adds draft-only pages: ${missingCashPathSystemPages.join(", ")}. Existing pages are unchanged.` : "All canonical CashPath pages already exist."}</p>
                </button>
                {missingGuides.length ? (
                  <button type="button" disabled={!canConfigure} onClick={installCashPathGuidesInDraft} className="mt-4 w-full rounded-[24px] border border-[#167a6a]/25 bg-[#e8f5f1] p-5 text-left text-[#182b29] disabled:opacity-50">
                    <p className="text-[10px] font-semibold uppercase tracking-[0.2em] text-[#167a6a]">CashPath · draft only</p>
                    <h3 className="mt-2 text-xl font-semibold">SYNC TO DRAFT · {missingGuides.length} {missingGuides.length === 1 ? "guide" : "guides"}</h3>
                    <p className="mt-2 text-sm text-[#182b29]/70">{missingGuideCategories.map(([category, guides]) => `${category} · ${guides.length}`).join(" · ")}</p>
                    <details className="mt-3 text-sm text-[#182b29]/70" onClick={(event) => event.stopPropagation()}>
                      <summary className="cursor-pointer font-semibold">Show missing guides</summary>
                      <ul className="mt-2 max-h-64 space-y-1 overflow-auto">
                        {missingGuideCategories.flatMap(([category, guides]) => guides.map((guide) => <li key={guide.slug}>{guide.title} · {guide.slug} · {category}</li>))}
                      </ul>
                    </details>
                    <p className="mt-2 text-sm text-[#182b29]/70">Adds only these missing guides to the draft. Existing pages stay unchanged; this does not publish the site.</p>
                  </button>
                ) : null}
                {needsCashPathGuideRepair ? <button type="button" disabled={!canConfigure} onClick={repairCashPathGuideRichTextInDraft} className="mt-4 w-full rounded-[24px] border border-[#167a6a]/25 bg-[#e8f5f1] p-5 text-left text-[#182b29] disabled:opacity-50"><p className="text-[10px] font-semibold uppercase tracking-[0.2em] text-[#167a6a]">CashPath</p><h3 className="mt-2 text-xl font-semibold">Исправить повреждённый текст SEO-гайда</h3><p className="mt-2 text-sm text-[#182b29]/70">Восстановит только повреждённые rich-text поля зарегистрированных SEO-гайдов. Остальные тексты, страницы и настройки не изменятся.</p></button> : null}
                ...
                {needsCashPathFormRepair ? <button type="button" disabled={!canConfigure} onClick={repairCashPathRequestFormInDraft} className="mt-4 w-full rounded-[24px] border border-[#167a6a]/25 bg-[#e8f5f1] p-5 text-left text-[#182b29] disabled:opacity-50"><p className="text-[10px] font-semibold uppercase tracking-[0.2em] text-[#167a6a]">CashPath</p><h3 className="mt-2 text-xl font-semibold">Восстановить форму заявки</h3><p className="mt-2 text-sm text-[#182b29]/70">Заменит старый текстовый блок CashPath на форму заявки. Остальные страницы и настройки не изменятся.</p></button> : null}
              </>
            ) : null}
          </div>
        </div>
