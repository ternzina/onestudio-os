...
export default function AdminSettingsPage() {
  ...
  async function saveGlobal(event: FormEvent) {
    ...
    setMessage(error ? error.message : t("Global settings saved."));
    setSaving(null);
  }
  ...
  async function saveContacts(event: FormEvent) {
    ...
    setMessage(error ? error.message : t("Contact settings saved."));
    setSaving(null);
  }
  ...
  async function saveBooking(event: FormEvent) {
    ...
    if (booking.service_open_hour >= booking.service_close_hour) {
      setMessage(t("Opening hour must be earlier than closing hour."));
      setSaving(null);
      return;
    }

    const { error } = await supabase.from("booking_page_settings").upsert({ id: "main", ...booking });
    setMessage(error ? error.message : t("Booking foundation settings saved."));
    setSaving(null);
  }
      ...
      <main className="min-h-screen px-5 pb-24 pt-36">
        <section className="mx-auto max-w-6xl">
          <div className="rounded-[36px] bg-[#17191f] p-7 text-white sm:p-10">
            <p className="text-xs uppercase tracking-[0.3em] text-[#d8b36a]">{t("Schema-aligned settings")}</p>
            <h1 className="mt-4 text-4xl font-semibold tracking-[-0.05em] sm:text-6xl">{t("Foundation settings")}</h1>
            <p className="mt-5 max-w-3xl text-sm leading-7 text-white/68">
              {t("These forms use the locale-neutral fields from the clean database migration. Old UA and PL columns are not used here.")}
            </p>
          </div>
            ...
            <div className="mt-6 grid gap-6">
              <SettingsCard title={t("Business identity")} description={t("Neutral name, logo text, footer and social links.")}>
                <form onSubmit={saveGlobal} className="grid gap-4 sm:grid-cols-2">
                  ...
                  <TextField label={t("Logo text")} value={global.logo_text} onChange={(value) => setGlobal({ ...global, logo_text: value })} />
                  <TextField label={t("Notification email")} type="email" value={global.notification_email} onChange={(value) => setGlobal({ ...global, notification_email: value })} />
                  <TextField label={t("Instagram URL")} value={global.instagram_url} onChange={(value) => setGlobal({ ...global, instagram_url: value })} />
                  <TextField label={t("TikTok URL")} value={global.tiktok_url} onChange={(value) => setGlobal({ ...global, tiktok_url: value })} />
                  <TextField label={t("Facebook URL")} value={global.facebook_url} onChange={(value) => setGlobal({ ...global, facebook_url: value })} />
                  <TextArea label={t("Footer text")} value={global.footer_text} onChange={(value) => setGlobal({ ...global, footer_text: value })} />
                  <SaveButton busy={saving === "global"} label={t("Save identity")} />
                </form>
              </SettingsCard>

              <SettingsCard title={t("Contact and business details")} description={t("Public contact information and optional transfer details.")}>
                <form onSubmit={saveContacts} className="grid gap-4 sm:grid-cols-2">
                  <TextField label={t("Phone")} value={contacts.phone} onChange={(value) => setContacts({ ...contacts, phone: value })} />
                  <TextField label={t("Email")} type="email" value={contacts.email} onChange={(value) => setContacts({ ...contacts, email: value })} />
                  <TextField label={t("Address")} value={contacts.address} onChange={(value) => setContacts({ ...contacts, address: value })} />
                  <TextField label={t("Opening hours")} value={contacts.hours} onChange={(value) => setContacts({ ...contacts, hours: value })} />
                  <TextField label={t("Map search query")} value={contacts.google_maps_query} onChange={(value) => setContacts({ ...contacts, google_maps_query: value })} />
                  <TextField label={t("Business ID or tax number")} value={contacts.nip} onChange={(value) => setContacts({ ...contacts, nip: value })} />
                  <TextField label={t("Transfer recipient")} value={contacts.transfer_recipient} onChange={(value) => setContacts({ ...contacts, transfer_recipient: value })} />
                  <TextField label={t("Transfer title")} value={contacts.transfer_title} onChange={(value) => setContacts({ ...contacts, transfer_title: value })} />
                  <TextField label={t("Bank account")} value={contacts.bank_account} onChange={(value) => setContacts({ ...contacts, bank_account: value })} />
                  <TextField label={t("Alternative payment ID")} value={contacts.blik} onChange={(value) => setContacts({ ...contacts, blik: value })} />
                  <TextField label={t("Instagram handle")} value={contacts.instagram} onChange={(value) => setContacts({ ...contacts, instagram: value })} />
                  <TextField label={t("Instagram URL")} value={contacts.instagram_url} onChange={(value) => setContacts({ ...contacts, instagram_url: value })} />
                  <SaveButton busy={saving === "contacts"} label={t("Save contacts")} />
                </form>
              </SettingsCard>

              <SettingsCard title={t("Booking foundation")} description={t("Only the universal switches and service hours that exist in the clean schema.")}>
                <form onSubmit={saveBooking} className="grid gap-4 sm:grid-cols-2">
                  <Toggle label={t("Service booking enabled")} checked={booking.service_booking_enabled} onChange={(value) => setBooking({ ...booking, service_booking_enabled: value })} />
                  <Toggle label={t("Resource booking enabled")} checked={booking.resource_booking_enabled} onChange={(value) => setBooking({ ...booking, resource_booking_enabled: value })} />
                  <NumberField label={t("Service opening hour")} value={booking.service_open_hour} onChange={(value) => setBooking({ ...booking, service_open_hour: value })} />
                  <NumberField label={t("Service closing hour")} value={booking.service_close_hour} onChange={(value) => setBooking({ ...booking, service_close_hour: value })} />
                  <TextField
                    label={t("Allowed durations in hours")}
                  ...
                  />
                  <SaveButton busy={saving === "booking"} label={t("Save booking settings")} />
                </form>
              </SettingsCard>
            </div>
          )}
