      ...
      <main className="min-h-screen px-5 pb-24 pt-36">
        <section className="mx-auto w-full max-w-7xl">
          <div className="rounded-[36px] bg-[#17191f] p-7 text-white shadow-[0_28px_90px_rgba(20,20,20,0.18)] sm:p-10">
            <p className="text-xs font-semibold uppercase tracking-[0.28em] text-[#d8b36a]">{t("Core Suite 1.0")}</p>
            <div className="mt-5 grid gap-8 lg:grid-cols-[1.25fr_0.75fr] lg:items-end">
              <div>
                ...
                <p className="mt-5 max-w-3xl text-sm leading-7 text-white/68 sm:text-base">
                  {t("The module registry now matches the database for every existing and future workspace.")}
                </p>
              </div>
              <div className="rounded-[26px] border border-white/10 bg-white/[0.07] p-5">
                <p className="text-xs uppercase tracking-[0.2em] text-[#d8b36a]">{t("Release status")}</p>
                <p className="mt-3 text-lg font-semibold">{t("Core plus nine modules enabled.")}</p>
                <p className="mt-2 text-sm leading-6 text-white/62">{t("Dependencies are explicit and workspace isolation remains enforced.")}</p>
              </div>
            </div>
          </div>
