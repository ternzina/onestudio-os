        ...
        <section className="mx-auto w-full max-w-7xl">
          <div className="rounded-[36px] bg-[#17191f] p-7 text-white shadow-[0_28px_90px_rgba(20,20,20,0.18)] sm:p-10">
            ...
            <div className="mt-5 grid gap-8 lg:grid-cols-[1.25fr_0.75fr] lg:items-end">
              <div>
                <h1 className="text-4xl font-semibold tracking-[-0.06em] sm:text-6xl">{t("One catalog, many business models.")}</h1>
                <p className="mt-5 max-w-3xl text-sm leading-7 text-white/68 sm:text-base">
                  {t("The catalog contract is now active in the admin. Services define appointments, rentals, classes and events, while resources define the capacity each offer needs.")}
                </p>
              </div>
              <div className="rounded-[26px] border border-white/10 bg-white/[0.07] p-5">
                <p className="text-xs uppercase tracking-[0.2em] text-[#d8b36a]">{t("Compatibility rule")}</p>
                <p className="mt-3 text-lg font-semibold">{t("Booking flows remain untouched.")}</p>
                <p className="mt-2 text-sm leading-6 text-white/62">{t("Scheduling and checkout will consume this catalog only after their own contracts and tests are complete.")}</p>
              </div>
            </div>
          </div>
