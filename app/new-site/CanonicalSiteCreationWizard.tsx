...
const templateGroups = getCustomerTemplateGroups();
...
export default function CanonicalSiteCreationWizard({ initialMode, initialTemplateKey }: { initialMode: TemplateCreationMode | null; initialTemplateKey: TemplateKey | null }) {
  ...
  return <main className="min-h-screen bg-[#0b0d12] px-5 py-10 text-white"><section className="mx-auto max-w-6xl rounded-[34px] border border-white/10 bg-white/[.04] p-6 sm:p-10">
    ...
    <form onSubmit={submit} className="mt-8">
      {step === 0 ? <div className="space-y-6"><article className={`rounded-[26px] border p-6 ${mode === "blank" && templateKey === "standard" ? "border-[#d8b36a] bg-[#d8b36a]/10 shadow-[0_0_0_1px_rgba(216,179,106,.2)]" : "border-white/10 bg-white/5"}`}><span className="text-xs uppercase tracking-[.2em] text-[#d8b36a]">Создать с нуля</span><strong className="mt-2 block text-2xl">Base OneStudio</strong><p className="mt-3 max-w-2xl text-sm leading-6 text-white/55">Нейтральная основа без готового бизнес-дизайна.<br />Все блоки, страницы и оформление можно собрать в редакторе.</p><button type="button" onClick={() => chooseTemplate("blank", "standard")} aria-pressed={mode === "blank" && templateKey === "standard"} className={`mt-5 rounded-full px-4 py-2 text-xs font-semibold ${mode === "blank" && templateKey === "standard" ? "bg-[#d8b36a] text-black" : "bg-white text-black"}`}>{mode === "blank" && templateKey === "standard" ? "✓ Выбрано" : "Начать с нуля"}</button></article>{(["free", "premium"] as const).map(access => templateGroups[access].length ? <section key={access} className="space-y-4" aria-labelledby={`template-group-${access}`}><div><p className="text-xs font-bold uppercase tracking-[.2em] text-[#d8b36a]">{access === "free" ? "Бесплатные" : "Premium"}</p><h2 id={`template-group-${access}`} className="mt-1 text-2xl font-semibold">{access === "free" ? "FREE шаблоны" : "PREMIUM шаблоны"}</h2></div><div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">{templateGroups[access].map(item => { const selected = templateKey === item.key && mode === "template"; return <article key={item.key} className={`overflow-hidden rounded-[26px] border ${selected ? "border-[#d8b36a] bg-[#d8b36a]/5 shadow-[0_0_0_1px_rgba(216,179,106,.2)]" : "border-white/10"}`}><div className="relative aspect-[4/3] bg-white/5">{item.gallery.previewImage ? <Image src={item.gallery.previewImage} alt="" fill className="object-cover" sizes="(max-width: 768px) 100vw, (max-width: 1024px) 50vw, 33vw" /> : null}</div><div className="p-5"><div className="flex items-start justify-between gap-3"><h3 className="text-xl font-semibold">{item.name}</h3><span className={`rounded-full px-3 py-1 text-[10px] font-bold uppercase ${item.access === "premium" ? "bg-[#d8b36a] text-black" : "bg-white/10 text-white/70"}`}>{templateAccessLabel(item.access, "ru")}</span></div><p className="mt-2 min-h-16 text-sm leading-6 text-white/55">{item.gallery.localizedDescription.ru}</p><div className="mt-5 flex flex-wrap gap-2">{item.gallery.previewRoute ? <Link href={item.gallery.previewRoute} target="_blank" rel="noreferrer" className="rounded-full border border-white/20 px-4 py-2 text-xs">Посмотреть демо</Link> : null}<button type="button" onClick={() => chooseTemplate("template", item.key)} aria-pressed={selected} className={`rounded-full px-4 py-2 text-xs font-semibold text-black ${selected ? "bg-[#d8b36a]" : "bg-white"}`}>{selected ? "✓ Выбрано" : "Выбрать шаблон"}</button></div></div></article>; })}</div></section> : null)}{hasSelectedFoundation ? <div role="status" className="rounded-2xl border border-[#d8b36a]/40 bg-[#d8b36a]/10 px-5 py-4 text-sm shadow-[0_12px_36px_rgba(0,0,0,.16)]"><p className="font-semibold text-[#e7ca92]">✓ Основа сайта выбрана</p><p className="mt-1 text-white/70">Нажмите «Продолжить», чтобы перейти к настройке бизнеса.</p></div> : null}</div> : null}
