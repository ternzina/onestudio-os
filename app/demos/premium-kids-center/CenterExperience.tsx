...
export function CenterExperience({ content }: { content?: PremiumKidsContent }) {
  ...
  const visibleTeachers = content?.teachers?.length ? content.teachers.map((item, index) => { const [name, role = "Педагог"] = item.split("·").map(part => part.trim()); return { ...centerTeachers[index % centerTeachers.length], name, role }; }) : centerTeachers;
  const visibleReviews = content?.reviews?.length ? content.reviews.map(item => { const parts = item.split("·"); return [parts.slice(0, -1).join("·").trim() || item, parts.at(-1)?.trim() || "Отзыв"] as const; }) : reviews;
  const visibleFaqs = content?.faq?.length ? content.faq.map(item => { const [question, ...answer] = item.split("·"); return [question.trim(), answer.join("·").trim()] as const; }) : faqs;

  return <div className={styles.centerExperience}>
    <section className={styles.centerPrograms} id="programs"><SectionLead index="07" eyebrow={content?.intro_eyebrow ?? "Программы по возрастам"} title={content?.intro_title ?? "Интерес растёт вместе с ребёнком"} text={content?.intro_description ?? "Выберите возраст — мы покажем направления, в которых сейчас будет особенно интересно."} />
    ...
    <section className={styles.centerGallery} id="gallery"><SectionLead index="11" eyebrow="Жизнь центра" title={content?.gallery_title ?? "Не постановка. Настоящий процесс открытия"} /><div>{gallery.map(([src, label, alt], index) => <figure key={src}><div><Image src={`/images/demos/premium-kids-center/${src}`} alt={alt} fill sizes="(max-width: 760px) 100vw, 50vw" /></div><figcaption>{content?.gallery_captions?.[index] ?? label} <Arrow /></figcaption></figure>)}</div></section>

    <section className={styles.centerReviews}><SectionLead index="12" eyebrow="Голос родителей" title={content?.reviews_title ?? "Спокойствие тоже можно почувствовать"} text="Все отзывы в этом блоке созданы исключительно как демонстрационный контент шаблона." /><div>{visibleReviews.map(([quote, author], index) => <blockquote key={`${author}-${index}`}><span aria-hidden="true">“</span><p>{quote}</p><footer>{author}</footer><i>0{index + 1}</i></blockquote>)}</div></section>

    <section className={styles.centerFaq} id="faq"><SectionLead index="13" eyebrow="FAQ" title={content?.faq_title ?? "Перед первым визитом"} /><div>{visibleFaqs.map(([question, answer], index) => { const open = openFaq === index; return <div className={styles.centerFaqItem} key={`${question}-${index}`}><h3><button aria-expanded={open} aria-controls={`faq-panel-${index}`} onClick={() => setOpenFaq(open ? null : index)}><span>0{index + 1}</span>{question}<i aria-hidden="true">+</i></button></h3><AnimatePresence initial={false}>{open ? <motion.div id={`faq-panel-${index}`} initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: reduced ? 0 : .28 }}><p>{answer}</p></motion.div> : null}</AnimatePresence></div>; })}</div></section>
  </div>;
}

export function CenterFinalCta({ content }: { content?: PremiumKidsContent }) {
  const ref = useRef<HTMLElement>(null);
  const active = useInView(ref, { amount: .2 });
  const reduced = useReducedMotion();
  return <section ref={ref} className={styles.centerFinalCta} id="trial"><div className={styles.centerFinalOrb} data-active={active && !reduced ? "true" : "false"} aria-hidden="true"><i /><i /><i /></div><p>{content?.final_cta_eyebrow ?? "BEMBI · Kids Discovery Platform"}</p><h2>{(content?.final_cta_title ?? "Первое открытие начинается\nс одного занятия.").split("\n").map((line, index) => <span key={`${line}-${index}`}>{line}{index === 0 ? <br /> : null}</span>)}</h2><a href="#schedule">{content?.final_cta_label ?? "Записаться на пробное занятие"} <Arrow /></a><span>Демонстрационный интерфейс OneStudio OS: форма не отправляет заявку и не создаёт бронирование.</span></section>;
}
