...
export function CenterFinalCta({ content, blockId, headingStyle }: { content?: PremiumKidsContent; blockId?: string; headingStyle?: CSSProperties }) {
  ...
  const reduced = useReducedMotion();
  return <section ref={ref} className={styles.centerFinalCta} id={!blockId || blockId === "bembi-final" ? "trial" : undefined} data-premium-block-id={blockId}><div className={styles.centerFinalOrb} data-active={active && !reduced ? "true" : "false"} aria-hidden="true"><i /><i /><i /></div><p>{content?.final_cta_eyebrow ?? "BEMBI · Kids Discovery Platform"}</p><h2 style={headingStyle}>{(content?.final_cta_title ?? "Первое открытие начинается\nс одного занятия.").split("\n").map((line, index) => <span key={`${line}-${index}`}>{line}{index === 0 ? <br /> : null}</span>)}</h2><a href="#schedule">{content?.final_cta_label ?? "Записаться на пробное занятие"} <Arrow /></a><span>Демонстрационный интерфейс OneStudio OS: форма не отправляет заявку и не создаёт бронирование.</span></section>;
}
