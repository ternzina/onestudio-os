...
test("template cards remain catalog-driven and responsive to future entries", async () => {
  ...
  assert.deepEqual(TEMPLATE_CATALOG.map(item => item.key), [
    "standard", "align-pilates-studio", "ritmo-dance-studio", "pawhaus-grooming-studio", "rastem-center", "gloss-nail-studio", "premium-kids-center", "lumea-beauty", "premium-studio", "velora-event-venue", "vow-films",
  ]);
});
