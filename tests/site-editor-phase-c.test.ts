...
describe("OneStudio editor design contract", () => {
  test("legacy BEMBI remains executable but is not an active editor design", () => {
    assert.deepEqual(getActiveEditorDesigns().map((design) => design.key), ["standard", "align-pilates-studio", "ritmo-dance-studio", "pawhaus-grooming-studio", "rastem-center", "gloss-nail-studio", "lumea-beauty", "premium-studio", "velora-event-venue", "vow-films"]);
