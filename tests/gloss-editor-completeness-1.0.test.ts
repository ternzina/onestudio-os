...
test("GLOSS generic fields edit nested settings without flattening unknown legacy data", () => {
  ...
  const edit = (section: keyof typeof GLOSS_EDITOR_EDITABLE_PATHS, id: string, value: string | boolean) => {
    ...
    else if (editor.type !== "notice" && editor.type !== "button" && editor.type !== "action" && editor.type !== "custom" && editor.type !== "composition" && editor.type !== "typography" && editor.type !== "richText" && editor.type !== "mediaList") editor.onChange(String(value));
  };
