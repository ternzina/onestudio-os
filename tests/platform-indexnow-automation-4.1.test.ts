...
test("platform URL mapping uses current public registries", async () => {
  ...
  assert.ok(platformUrlsForChangedFiles(["lib/seo/journal-articles.ts"]).includes("https://onestudioos.com/journal"));
  assert.ok(platformUrlsForChangedFiles(["lib/seo/journal-articles.ts"]).every((url: string) => !url.includes("/blog")));
