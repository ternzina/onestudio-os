...
type ShortFormValues = { requested_amount: string; email: string; zip: string };
...
function CashPathShortForm({ onContinue, preview }: { onContinue: (values: ShortFormValues) => void; preview: boolean }) {
  const [values, setValues] = useState<ShortFormValues>({ requested_amount: "", email: "", zip: "" });
  const [error, setError] = useState("");
  const update = (key: keyof ShortFormValues, value: string) => setValues((current) => ({ ...current, [key]: value }));
  const submit = (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    if (preview) return;
    if (!values.requested_amount || !/^\S+@\S+\.\S+$/.test(values.email) || !/^\d{5}(?:-\d{4})?$/.test(values.zip)) {
      setError("Enter a loan amount, a valid email address, and a ZIP code to continue.");
      return;
    }
    onContinue(values);
  };
  return <form className="grid gap-4 sm:grid-cols-2" onSubmit={submit} noValidate>
    <label className="grid gap-2 text-sm font-semibold text-[#24302d]">Loan amount<input aria-label="Loan amount" inputMode="numeric" placeholder="$1,000" value={values.requested_amount} onChange={(event) => update("requested_amount", event.target.value)} disabled={preview} className="rounded-xl border border-black/15 bg-white px-4 py-3 text-base font-normal outline-none ring-[#187c6a] focus:ring-2 disabled:cursor-not-allowed disabled:bg-black/5" /></label>
    ...
    <label className="grid gap-2 text-sm font-semibold text-[#24302d]">ZIP code<input aria-label="ZIP code" inputMode="numeric" autoComplete="postal-code" placeholder="00000" value={values.zip} onChange={(event) => update("zip", event.target.value)} disabled={preview} className="rounded-xl border border-black/15 bg-white px-4 py-3 text-base font-normal outline-none ring-[#187c6a] focus:ring-2 disabled:cursor-not-allowed disabled:bg-black/5" /></label>
    <div className="flex items-end"><button type="submit" disabled={preview} className="w-full rounded-xl bg-[#187c6a] px-5 py-3 text-base font-bold text-white transition hover:bg-[#126657] disabled:cursor-not-allowed disabled:opacity-70">Continue</button></div>
