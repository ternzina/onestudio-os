...
import { resolvePremiumTemplatePlainText } from "@/lib/public-site/premium-template-field-contract";
...
function Action({ section, id, action, className = "" }: { section: string; id: string; action: { title: string; text: string }; className?: string }) {
  return <a className={`${styles.action} ${className}`} href={action.text} data-premium-action={key(section, id)}>{resolvePremiumTemplatePlainText(action.title)}<span aria-hidden="true">↗</span></a>;
}

function Media({ src, label, className = "" }: { src?: string; label: string; className?: string }) {
  return <div className={`${styles.media} ${className}`} style={src ? { backgroundImage: `url(${src})` } : undefined} role="img" aria-label={resolvePremiumTemplatePlainText(label)} />;
}

function SectionHeading({ meta, title, text, style }: { meta: string; title: string; text?: string; style?: CSSProperties }) {
  return <div className={styles.heading}><small className={styles.eyebrow}>{meta}</small><h2 style={style}><PublicRichHeading value={title} /></h2>{text && <PublicRichText value={text} className={styles.intro} />}</div>;
}
  ...
  return <main className={styles.page} lang={en ? "en" : "ru"}>
    ...
    <Composition site={site}>
      <section id="top" data-editor-anchor="hero" className={styles.hero}>
        <div className={styles.heroCopy}><small className={styles.eyebrow}>{c.hero.eyebrow}</small><h1 style={heading("hero")}><PublicRichHeading value={c.hero.title} /></h1><PublicRichText value={c.hero.text} className={styles.lead} /><div className={styles.actions}><Action section="hero" id="primary" action={c.hero.primary} /><Action section="hero" id="secondary" action={c.hero.secondary} className={styles.textAction} /></div><div className={styles.heroFoot}><span>01 — 07</span><span>{en ? "Seasonal flowers, considered slowly." : "Сезонные цветы, собранные с вниманием."}</span></div></div>
