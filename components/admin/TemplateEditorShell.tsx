...
export type TemplateEditorSection = {
  ...
  label: string;
  description: string;
  capabilities: { visibility: boolean; reorder: boolean; duplicate: boolean; typography: boolean };
};

export default function TemplateEditorShell({ templateName, draftLabel, previewHref, sections, selectedSection, device, editingEnabled, saving, canUndo, canRedo, onSelectSection, onDeviceChange, onEditingChange, onUndo, onRedo, onSave, onPublish, navigator, canvas, inspector }: {
  ...
  navigator?: ReactNode; canvas: ReactNode; inspector: ReactNode;
}) {
  return <section id="site-builder-canvas" className="relative mt-8 scroll-mt-24 overflow-hidden rounded-[28px] border border-black/10 bg-[#e9e8e4] text-[#17191f] shadow-[0_26px_90px_rgba(25,27,32,0.12)]">
    ...
    <div className="grid min-h-[760px] lg:grid-cols-[230px_minmax(0,1fr)_330px]">
      <aside className="border-r border-black/10 bg-white/80 p-4"><p className="mb-3 text-[10px] font-semibold uppercase tracking-[0.18em] text-[#8b877e]">Секции страницы</p><nav className="space-y-1">{sections.map((section,index) => <button type="button" key={section.id} aria-current={selectedSection === section.id ? "true" : undefined} onClick={() => onSelectSection(section.id)} className={`flex w-full items-center gap-3 rounded-xl px-3 py-3 text-left text-xs font-semibold ${selectedSection === section.id ? "bg-[#17191f] text-white" : "hover:bg-black/5"}`}><span className="text-[10px] opacity-55">{String(index + 1).padStart(2,"0")}</span><span>{section.label}</span></button>)}</nav>{navigator}</aside>
