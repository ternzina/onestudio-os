      ...
      >
        ...
        <div className="flex min-h-full flex-col">
          <Link href="/admin" className="rounded-[26px] border border-black/8 bg-[#17191f] p-5 pr-14 text-white">
            <p className="text-[12px] font-semibold uppercase tracking-[0.2em] text-[#d8b36a]">OneStudio OS</p>
            <h2 className="mt-2 text-[26px] font-semibold tracking-[-0.05em]">OneStudio OS</h2>
            <p className="mt-2 text-[14px] leading-6 text-white/70">{t("Workspace")}</p>
          </Link>
            ...
            {activeItems.map((item) => {
              ...
              return (
                <Link
                  key={item.href}
                  href={item.href}
                  className={`flex items-center gap-3 rounded-[18px] px-4 py-3.5 text-[16px] font-semibold transition ${
