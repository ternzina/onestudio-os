...
export default function PremiumTemplateEditor({ businessId, businessSlug, businessName, locale, draft, disabled, saving, hasUnsavedChanges, device, canUndo, canRedo, onChange, onDeviceChange, onUndo, onRedo, onSave, onPublish }: {
  ...
  function add(type: PremiumKidsBlockType) { const id = newBlockId(type); const next = addPremiumKidsBlock(premium, type, id); if (next !== premium) { commit(next); setShowLibrary(false); selectBlock(id); } }
  ...
  return <TemplateEditorShell templateName="BEMBI Premium" draftLabel={`Черновик${hasUnsavedChanges ? " · не сохранён" : " · сохранён"}`} previewHref={`/site-preview/premium-kids-center/${businessSlug}`} sections={sections} selectedSection={selected} device={device} editingEnabled={editingEnabled} saving={saving || disabled} canUndo={canUndo} canRedo={canRedo} onSelectSection={selectBlock} onDeviceChange={onDeviceChange} onEditingChange={setEditingEnabled} onUndo={onUndo} onRedo={onRedo} onSave={onSave} onPublish={onPublish}
    ...
    navigator={<div className="mt-4"><button type="button" disabled={controlsDisabled} onClick={() => setShowLibrary(value => !value)} className="w-full rounded-xl border border-dashed border-[#9a742e]/50 bg-[#fffaf0] px-3 py-2.5 text-xs font-semibold text-[#76551d] disabled:opacity-40">+ Добавить блок</button>{showLibrary ? <div className="mt-2 grid gap-1 rounded-xl border border-black/8 bg-white p-2">{PREMIUM_KIDS_BLOCK_REGISTRY.filter(item => item.capabilities.add).map(item => <button type="button" key={item.type} onClick={() => add(item.type)} className="rounded-lg px-2 py-2 text-left text-xs hover:bg-black/5"><b className="block">{item.label}</b><span className="text-[10px] text-[#716d65]">{item.description}</span></button>)}</div> : null}<p className="mt-3 text-[10px] leading-5 text-[#716d65]">◆ обязательный блок · ⠿ можно перемещать<br />Скрытые блоки остаются доступны для редактирования.</p></div>}
    ...
    inspector={<><p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-[#9a742e]">Настройки блока</p><h3 className="mt-2 text-xl font-semibold">{selectedSection.label}</h3><p className="mt-1 text-xs leading-5 text-[#716d65]">{definition.description}</p><div className="mt-5 grid gap-4">
      ...
      {fields[selectedBlock.type].map(([key, label, kind]) => { const raw = selectedBlock.props[key]; const value = Array.isArray(raw) ? raw.join("\n") : raw ?? ""; return <label key={key} className="text-xs font-semibold text-[#4f4b45]">{label}{kind === "input" ? <input className={inputClass} value={value} disabled={controlsDisabled} onChange={event => update(key, event.target.value)} /> : kind === "text" && ["hero_description", "intro_description", "programs_description", "schedule_description", "footer_description"].includes(key) ? <RichTextEditor value={value} disabled={controlsDisabled} onChange={next => update(key, next)} /> : <textarea className={inputClass} rows={kind === "lines" ? 6 : 3} value={value} disabled={controlsDisabled} onChange={event => update(key, event.target.value)} />}</label>; })}
      {definition.capabilities.typography ? <TypographyControls title="Заголовок секции" description="Ограниченные настройки Site Editor 2.6" value={selectedBlock.props.heading_typography} disabled={controlsDisabled} onChange={updateTypography} /> : null}
