...
import { sitemapEligiblePageCount } from "@/lib/public-site/search-visibility";
...
export function OneStudioSeoDialog({ open, businessId, draft, canConfigure, saving, publicOrigin, indexNowConfigured, onChange, onSave, onClose }: SeoDialogProps) {
  const { t } = useAdminI18n();
  ...
  const indexingEnabled = draft.seo_no_index !== true;
  const sitemapCount = sitemapEligiblePageCount(draft);
  const robotsHref = new URL("/robots.txt", publicOrigin).toString();
  const sitemapHref = new URL("/sitemap.xml", publicOrigin).toString();
