...
export function OneStudioEditorWorkspace({ children, navigatorOpen, settingsOpen }: { children: ReactNode; navigatorOpen: boolean; settingsOpen: boolean }) {
  return <div data-onestudio-editor-workspace data-template-editor-columns data-navigator-open={String(navigatorOpen)} data-settings-open={String(settingsOpen)} className={`relative grid min-w-0 overflow-hidden ${TEMPLATE_EDITOR_COLUMNS_CLASS}`}>{children}</div>;
}
...
function EditorSettingsFrame({ children, className = "", id }: { children: ReactNode; className?: string; id?: string }) {
  return <aside id={id} data-template-editor-settings className={`min-w-0 overflow-y-auto overscroll-contain border-l border-black/10 bg-white p-4 [scrollbar-gutter:stable] ${className}`}>{children}</aside>;
}
...
export default function OneStudioSiteEditor({ canvasRef, canvasProps, ...spec }: TemplateEditorSpec) {
  ...
  const [internalNavigatorOpen, setInternalNavigatorOpen] = useState(true);
  const [internalSettingsOpen, setInternalSettingsOpen] = useState(true);
  const navigatorOpen = spec.navigatorModel.expanded ?? internalNavigatorOpen;
  const settingsOpen = spec.inspectorModel.expanded ?? internalSettingsOpen;
  const setNavigatorOpen = (open: boolean) => { setInternalNavigatorOpen(open); spec.navigatorModel.onExpandedChange?.(open); };
  const setSettingsOpen = (open: boolean) => { setInternalSettingsOpen(open); spec.inspectorModel.onExpandedChange?.(open); };
  const navigatorModel = { ...spec.navigatorModel, onCollapse: () => { setCompactPanel(null); setNavigatorOpen(false); spec.navigatorModel.onCollapse?.(); } };
  const inspectorModel = { ...spec.inspectorModel, onCollapse: () => { setCompactPanel(null); setSettingsOpen(false); spec.inspectorModel.onCollapse?.(); } };
  return <section id="site-builder-canvas" data-template-editor-runtime data-template-key={spec.templateKey} className="relative mt-8 w-full scroll-mt-24 overflow-hidden rounded-[28px] border border-black/10 bg-[#e9e8e4] text-[#17191f] shadow-[0_26px_90px_rgba(25,27,32,0.12)]">
    <OneStudioEditorToolbar>
      ...
      <OneStudioEditorCommandBar spec={spec} navigatorOpen={navigatorOpen} settingsOpen={settingsOpen} onBlocks={() => setNavigatorOpen(!navigatorOpen)} onSettings={() => setSettingsOpen(!settingsOpen)} />
    </OneStudioEditorToolbar>
    <div className="template-editor-compact-controls items-center gap-2 border-b border-black/10 bg-white/90 p-3">
      <button type="button" aria-expanded={compactPanel === "navigator"} aria-controls="site-editor-blocks-panel" onClick={() => setCompactPanel(current => current === "navigator" ? null : "navigator")} className="flex-1 rounded-xl border border-black/10 bg-white px-3 py-2 text-xs font-semibold">{t("Blocks")}</button>
      <button type="button" aria-expanded={compactPanel === "settings"} aria-controls="site-editor-settings-panel" onClick={() => setCompactPanel(current => current === "settings" ? null : "settings")} className="flex-1 rounded-xl border border-black/10 bg-white px-3 py-2 text-xs font-semibold">{t("Settings")}</button>
    </div>
    <div data-compact-panel={compactPanel ?? "canvas"}>
      <OneStudioEditorWorkspace navigatorOpen={navigatorOpen} settingsOpen={settingsOpen}>
        <EditorNavigatorFrame id="site-editor-blocks-panel" className={TEMPLATE_EDITOR_NAVIGATOR_CLASS}><SharedEditorNavigator model={navigatorModel} /></EditorNavigatorFrame>
        <EditorCanvasFrame ref={canvasRef} className={TEMPLATE_EDITOR_CANVAS_CLASS} {...canvasProps}>{spec.canvas}</EditorCanvasFrame>
        <EditorSettingsFrame id="site-editor-settings-panel" className={TEMPLATE_EDITOR_SETTINGS_CLASS}><SharedEditorInspector model={inspectorModel} /></EditorSettingsFrame>
