...
function printDocument(document: DocumentRow) {
  const escape = (value: string) => value
    ...
    .replaceAll('"', "&quot;")
    .replaceAll("'", "&#039;");
  const html = `<!doctype html><html><head><meta charset="utf-8"><title>${escape(document.title_snapshot)}</title><style>body{font-family:Arial,sans-serif;color:#17191f;margin:48px;line-height:1.65}h1{font-size:28px;margin:0 0 28px}.meta{color:#77736a;font-size:12px;margin-bottom:30px}.body{white-space:pre-wrap;font-size:15px}@media print{body{margin:20mm}}</style></head><body><h1>${escape(document.title_snapshot)}</h1><div class="meta">${escape(document.document_number)}</div><div class="body">${escape(document.content_snapshot || "Document content is empty.")}</div><script>window.addEventListener("load",function(){setTimeout(function(){window.focus();window.print();},300);});<\/script></body></html>`;
