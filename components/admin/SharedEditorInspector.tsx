...
function SharedEditorInspectorField({ field }: { field: EditorInspectorField }) {
  ...
  if (field.type === "textarea") return <label className="text-xs font-semibold text-[#4f4b45]">{field.label}<textarea className={editorCompactFieldClass} rows={field.rows ?? 3} value={field.value} disabled={field.disabled} onChange={event => field.onChange(event.target.value)} /></label>;
...
export default function SharedEditorInspector({ model }: { model: EditorInspectorModel }) {
  ...
  return <div data-shared-editor-inspector>
    ...
    <div data-editor-inspector-groups className="mt-6 grid gap-5">{ONESTUDIO_INSPECTOR_GROUPS.map(group => {
      ...
      return fields.length ? <section key={group} data-editor-inspector-group={group} className="rounded-2xl border border-black/8 bg-[#faf9f6] p-4"><h3 className="mb-3 text-xs font-semibold text-[#292722]">{labels[group]}</h3><div className="grid gap-4">{fields.map(field => <SharedEditorInspectorField key={field.id} field={field} />)}</div></section> : null;
    })}</div>
