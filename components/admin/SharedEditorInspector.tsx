...
export function SharedEditorInspectorField({ field, headingRichText = false }: { field: EditorInspectorField; headingRichText?: boolean }) {
  const isHeading = field.semantic === "heading" || (field.semantic === undefined && headingRichText);
  ...
  if (field.type === "richText") return <SiteEditorTextField label={field.label} value={field.value} originalValue={field.originalValue} disabled={field.disabled ?? false} richText={!isHeading} headingRichText={isHeading} onChange={field.onChange} />;
  ...
  if (field.type === "textarea") return <SiteEditorTextField label={field.label} value={field.value} originalValue={field.originalValue} disabled={field.disabled} multiline={!isHeading} headingRichText={isHeading} rows={field.rows ?? 3} onChange={field.onChange} />;
  if (field.type === "text") return <SiteEditorTextField label={field.label} value={field.value} originalValue={field.originalValue} disabled={field.disabled} headingRichText={isHeading} onChange={field.onChange} />;
...
function fieldsForInspectorGroup(fields: readonly EditorInspectorPlacedField[], group: OneStudioInspectorGroup) {
  ...
  const typographyByField = new Map<string, EditorInspectorPlacedField[]>();
  for (const field of fields) {
    if (field.type !== "typography" || !field.forFieldId || !knownFieldIds.has(field.forFieldId)) continue;
    typographyByField.set(field.forFieldId, [...(typographyByField.get(field.forFieldId) ?? []), field]);
  }
  return fields
    .filter(field => field.group === group && !(field.type === "typography" && field.forFieldId && knownFieldIds.has(field.forFieldId)))
    .flatMap(field => [field, ...(typographyByField.get(field.id) ?? [])]);
}
...
export default function SharedEditorInspector({ model }: { model: EditorInspectorModel }) {
  ...
  return <div data-shared-editor-inspector>
    ...
    <div data-editor-inspector-groups className="mt-6 grid gap-5">{ONESTUDIO_INSPECTOR_GROUPS.map(group => {
      const fields = fieldsForInspectorGroup(model.fields, group);
