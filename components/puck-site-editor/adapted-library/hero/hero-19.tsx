...
import {
  hero19ContentDefaults,
  type Hero19Invoice,
} from "../../content-editability-batch-4-contracts";

export type AdaptedHero19Props = {
  eyebrow: string;
  heading: string;
  description: string;
  primaryButtonLabel: string;
  secondaryButtonLabel: string;
  securityTrustLabel: string;
  launchTrustLabel: string;
  billingRunLabel: string;
  billingRunMeta: string;
  billingStatusLabel: string;
  recognizedLabel: string;
  recognizedValue: string;
  recognizedPercent: number;
  recognizedPlanLabel: string;
  paymentReceivedLabel: string;
  paymentReceivedDetail: string;
  approvalCompleteLabel: string;
  approvalCompleteDetail: string;
  invoices: readonly Hero19Invoice[];
};
...
export function AdaptedHero19({
  ...
  approvalCompleteDetail,
  invoices,
}: AdaptedHero19Props = hero19ContentDefaults) {
  ...
  const progress = Math.max(0, Math.min(100, recognizedPercent));
                ...
                >
                  ...
                  <div className="mt-3 h-1.5 overflow-hidden rounded-full bg-neutral-200 dark:bg-neutral-800">
                    <motion.div
                      ...
                      className="h-full origin-left rounded-full bg-neutral-950 dark:bg-white"
                    />
                  </div>
                  <p className="mt-2 text-xs text-neutral-500 dark:text-neutral-400">
                    {progress}% {recognizedPlanLabel}
                  </p>
                </motion.div>
              </div>
            </motion.div>
