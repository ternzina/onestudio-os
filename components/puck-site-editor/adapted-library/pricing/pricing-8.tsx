...
export const pricing8ContentDefaults = {
...
export type AdaptedPricing8Props = { [K in keyof typeof pricing8ContentDefaults]: string };
...
export default function AdaptedPricing8({ headingLine1, headingLine2, standardButtonLabel, enterpriseButtonLabel, monitoringTitle, incidentTitle }: AdaptedPricing8Props) {
  return (
    <section className="w-full min-h-[var(--rb-section-min-h,100vh)] py-16 px-4 sm:px-6 lg:px-8 bg-white dark:bg-neutral-950">
      <div className="max-w-[1400px] mx-auto w-full">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 items-start">
          ...
          {tiers.map((t, i) => (
            <div key={t} className="flex flex-col gap-4">
              ...
              >
                {i === 2 ? enterpriseButtonLabel : standardButtonLabel}
              </button>
            </div>
          ))}
        </div>
        ...
        <div className="mt-10">
          <Section title={monitoringTitle} rows={monitoringRows} />
          <Section title={incidentTitle} rows={incidentRows} />
        </div>
      </div>
    </section>
  );
