...
import {
  hero6SlidesDefaults,
  type Hero6Slide,
} from "./hero-6-contract";

export { hero6SlidesDefaults } from "./hero-6-contract";
export type { Hero6Slide } from "./hero-6-contract";

export type AdaptedHero6Props = { slides: readonly Hero6Slide[] };

export function AdaptedHero6({ slides }: AdaptedHero6Props) {
  const [currentSlide, setCurrentSlide] = useState(0);
  const [overallProgress, setOverallProgress] = useState(0);
  const isFirstRender = useRef(true);
  const resolvedSlides = slides.length > 0 ? slides : hero6SlidesDefaults;
  const activeSlideIndex = Math.min(currentSlide, resolvedSlides.length - 1);
  const activeSlide = resolvedSlides[activeSlideIndex];
  ...
  useEffect(() => {
    const progressInterval = setInterval(() => {
      setOverallProgress((prev) => {
        ...
        return prev + 100 / ((slideDuration * resolvedSlides.length) / 50);
      });
    }, 50);
  ...
  useEffect(() => {
    const slideIndex = Math.floor((overallProgress / 100) * resolvedSlides.length);
    setCurrentSlide(Math.min(slideIndex, resolvedSlides.length - 1));
  }, [overallProgress, resolvedSlides.length]);

  const handleSlideClick = (index: number) => {
    setOverallProgress((index / resolvedSlides.length) * 100);
              ...
              >
                ...
                {index === activeSlideIndex && (
                  <motion.div
                    ...
                    animate={{
                      height: `${
                        ((overallProgress - (index * 100) / resolvedSlides.length) /
                          (100 / resolvedSlides.length)) *
                        100
                      }%`,
                    }}
          ...
          <div className="flex items-center justify-center relative">
            <AnimatePresence mode="wait">
              <motion.div
                ...
                className={`absolute inset-0 blur-[100px] ${activeSlide.color} rounded-full opacity-20 dark:opacity-10`}
              />
            </AnimatePresence>
