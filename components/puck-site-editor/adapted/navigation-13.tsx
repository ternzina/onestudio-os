...
import {
  type AdaptedNavigation13Link,
  type AdaptedNavigation13Props,
} from "./navigation-13-contract";

export type { AdaptedNavigation13Link, AdaptedNavigation13Props } from "./navigation-13-contract";
export { navigation13Defaults } from "./navigation-13-contract";
...
function RollLink({ link }: { link: AdaptedNavigation13Link }) {
  return (
    <motion.a
      href={link.href ?? "#"}
      initial="rest"
...
export default function AdaptedNavigation13({
  links,
  brandName,
  brandHref,
  primaryActionLabel,
          ...
          <div className="mx-auto grid w-full max-w-[1400px] grid-cols-[1fr_auto_1fr] items-center gap-4 py-5">
            <div className="hidden items-center gap-8 lg:flex">
              {leftLinks.map((link, index) => (
                <RollLink key={`${link.label}-${index}`} link={link} />
              ))}
            </div>
            <button
              type="button"
            ...
            >
              {brandName}<span className="align-super text-[10px]">®</span>
            </a>
                ...
                >
                  {links.map((link, index) => (
                    ...
                    >
                      ...
                      <ArrowUpRight className="ml-auto h-5 w-5 self-center text-neutral-400 transition-colors group-hover:text-neutral-950 dark:text-neutral-600 dark:group-hover:text-white" />
                    </motion.a>
                  ))}
                </motion.nav>
