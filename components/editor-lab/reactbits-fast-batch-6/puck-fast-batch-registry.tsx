...
], fields: { title: fields.text("Plan name", { contentEditable: false }), description: fields.textarea("Plan description", { contentEditable: false }), price: fields.text("Price", { contentEditable: false }), suffix: fields.text("Price suffix", { contentEditable: false }), limit: fields.text("Usage limit", { contentEditable: false }), ctaLabel: fields.text("Button text", { contentEditable: false }) }, itemLabel: (item, index) => `Plan ${Number(index ?? 0) + 1} — ${String(item.title ?? "Untitled")}` });
