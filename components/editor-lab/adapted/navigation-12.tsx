          ...
          <div className="hidden items-center md:flex">
            {links.map((link, index) => (
              <button
                ...
                className={`relative cursor-pointer rounded-full px-4 py-2 text-sm font-medium transition-colors duration-200 ${focusRing} ${active === link.label ? "text-neutral-900 dark:text-white" : "text-neutral-500 hover:text-neutral-900 dark:text-neutral-400 dark:hover:text-white"}`}
              >
                {active === link.label && <motion.span layoutId="nav12-active" className="absolute inset-0 rounded-full bg-neutral-100 dark:bg-neutral-800" transition={{ duration: 0.35, ease: [0.22, 1, 0.36, 1] }} />}
              ...
              <motion.nav initial="hidden" animate="visible" variants={menuStagger} className="mt-3 rounded-3xl border border-neutral-200/80 bg-white/90 p-2 shadow-[0_8px_24px_-12px_rgba(0,0,0,0.12)] backdrop-blur-xl dark:border-neutral-800 dark:bg-neutral-900/90" aria-label="Mobile">
                {links.map((link, index) => (
                  <motion.button
                    ...
                    className={`flex w-full cursor-pointer items-center justify-between rounded-2xl px-4 py-3.5 text-left text-[15px] font-medium transition-colors duration-200 ${focusRing} ${active === link.label ? "bg-neutral-100 text-neutral-900 dark:bg-neutral-800 dark:text-white" : "text-neutral-600 hover:bg-neutral-50 dark:text-neutral-400 dark:hover:bg-neutral-800/50"}`}
                  >
                    {link.label}
                    {active === link.label && <span className="h-1.5 w-1.5 rounded-full bg-neutral-900 dark:bg-white" />}
                  </motion.button>
                ))}
