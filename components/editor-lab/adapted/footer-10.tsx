...
export type AdaptedFooter10Props = {
  lifecycleLinks: MenuLinkItem[];
  productLinks: MenuLinkItem[];
  supportLinks: MenuLinkItem[];
};

export const footer10LifecycleLinks: MenuLinkItem[] = [
  { label: "Start", href: "#" },
  { label: "Plan", href: "#" },
  { label: "Ship", href: "#" },
  { label: "Measure", href: "#" },
...
export const footer10ProductLinks: MenuLinkItem[] = [
  { label: "Home", href: "#" },
  { label: "Platform", href: "#" },
  { label: "Pricing", href: "#" },
  { label: "Careers", href: "#" },
...
export const footer10SupportLinks: MenuLinkItem[] = [
  { label: "Docs", href: "#" },
  { label: "Privacy", href: "#" },
  { label: "Terms", href: "#" },
  { label: "Contact", href: "#" },
...
export default function AdaptedFooter10({
  lifecycleLinks,
  productLinks,
  supportLinks,
}: AdaptedFooter10Props) {
    ...
    <footer className="relative w-full overflow-hidden bg-white px-4 py-16 dark:bg-neutral-950 sm:px-6 sm:py-20 lg:px-8 lg:py-24">
      ...
      >
        <div className="flex flex-col gap-14 lg:justify-between">
          <motion.h2 variants={item} className="max-w-2xl text-4xl font-medium leading-[1.05] tracking-[-0.03em] text-neutral-900 dark:text-white sm:text-5xl lg:text-6xl">
            Every moving part.
            <span className="block text-neutral-400 dark:text-neutral-500">One calm surface.</span>
          </motion.h2>

          <motion.nav variants={item} aria-label="Footer">
            <div className="flex flex-wrap items-baseline gap-x-5 gap-y-2">
              <span className="text-xs font-medium uppercase tracking-[0.2em] text-neutral-400 dark:text-neutral-500">Lifecycle</span>
              {lifecycleLinks.map((link, index) => (
                <a key={`${link.label}-${link.href}-${index}`} href={link.href ?? "#"} className={`rounded-sm text-sm font-medium text-neutral-800 transition-colors duration-200 hover:text-neutral-500 dark:text-neutral-200 dark:hover:text-neutral-400 ${focusRing}`}>
                  {link.label}
                </a>
              ))}
            </div>
            <div className="mt-8 grid max-w-sm grid-cols-2 gap-x-12">
              {[productLinks, supportLinks].map((group, groupIndex) => (
                <ul key={groupIndex} className="space-y-3.5">
                  {group.map((link, index) => (
                    <li key={`${link.label}-${link.href}-${index}`}>
                      <a href={link.href ?? "#"} className={`rounded-sm text-[15px] font-medium text-neutral-800 transition-colors duration-200 hover:text-neutral-500 dark:text-neutral-200 dark:hover:text-neutral-400 ${focusRing}`}>
                        {link.label}
                      </a>
                    </li>
                  ))}
          ...
          <motion.div variants={item} className="space-y-6">
            <div className="flex items-center gap-2">
              <a href="#" aria-label="X profile" className={`flex h-10 w-10 items-center justify-center rounded-xl border border-neutral-200 text-neutral-600 transition-colors duration-200 hover:border-neutral-300 hover:bg-neutral-50 hover:text-neutral-900 dark:border-neutral-800 dark:text-neutral-400 dark:hover:border-neutral-700 dark:hover:bg-neutral-900 dark:hover:text-white ${focusRing}`}>
              ...
              <a href="#" aria-label="LinkedIn profile" className={`flex h-10 w-10 items-center justify-center rounded-xl border border-neutral-200 text-neutral-600 transition-colors duration-200 hover:border-neutral-300 hover:bg-neutral-50 hover:text-neutral-900 dark:border-neutral-800 dark:text-neutral-400 dark:hover:border-neutral-700 dark:hover:bg-neutral-900 dark:hover:text-white ${focusRing}`}>
        ...
        >
          <img src="/svg/placeholder.svg" alt="" className="absolute inset-0 h-full w-full object-cover" />
          <div className="absolute inset-x-4 bottom-4 rounded-2xl border border-white/50 bg-white/85 p-5 backdrop-blur-md dark:border-white/10 dark:bg-neutral-950/80 sm:inset-x-5 sm:bottom-5 sm:p-6">
            <p className="text-[11px] font-medium uppercase tracking-[0.16em] text-neutral-500 dark:text-neutral-400">Inside Meridian</p>
            <p className="mt-3 max-w-sm text-lg font-medium leading-snug tracking-tight text-neutral-900 dark:text-white sm:text-xl">An orchestration desk built to run the entire operation, end to end.</p>
