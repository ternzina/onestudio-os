...
import { resolveIconToken } from "@/components/editor-lab/puck/icon-token-contract";
...
export type AppSidebar7IconToken = keyof typeof appSidebar7Icons;
export type AppSidebar7Group = {
  label: string;
  items: Array<{
    label: string;
    iconToken: AppSidebar7IconToken;
    current?: boolean;
    badge?: string;
  }>;
};

export const appSidebar7Groups: AppSidebar7Group[] = [
  {
    label: "Favorites",
    items: [
      { label: "Q3 pipeline review", iconToken: "board" },
      { label: "Renewals board", iconToken: "folder" },
    ],
  },
  {
    label: "Workspace",
    items: [
      { label: "Overview", iconToken: "overview", current: true },
      { label: "Pipelines", iconToken: "board", badge: "9" },
      { label: "Projects", iconToken: "folder" },
      { label: "Documents", iconToken: "document" },
      { label: "Customers", iconToken: "users" },
    ],
  },
  {
    label: "Reporting",
    items: [
      { label: "Performance", iconToken: "gauge" },
      { label: "Reports", iconToken: "reports" },
      { label: "Security", iconToken: "security" },
    ],
  },
  {
    label: "Account",
    items: [
      { label: "Members", iconToken: "users" },
      { label: "Billing", iconToken: "billing" },
      { label: "Settings", iconToken: "settings" },
    ],
  },
];

function SidebarBody({ groups, onClose }: { groups: AppSidebar7Group[]; onClose?: () => void }) {
              ...
              <div key={group.label}>
                ...
                <ul className="space-y-0.5">
                  {group.items.map((item) => {
                    const Icon = resolveIconToken(item.iconToken, appSidebar7Icons);
...
export default function AdaptedAppSidebar7({
  groups = appSidebar7Groups,
}: {
  groups?: AppSidebar7Group[];
}) {
    ...
    <div className="relative flex h-full min-h-[720px] w-full overflow-hidden bg-white dark:bg-neutral-950">
      ...
      {mobileOpen && (
        <div className="lg:hidden">
          ...
          >
            <SidebarBody groups={groups} onClose={closeMobile} />
          </aside>
        </div>
      )}
    </div>
