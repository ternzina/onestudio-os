...
export const pricing12Features = [
...
export const pricing12Icons = {
  shield: ShieldCheck,
  refresh: RefreshCcw,
  card: CreditCard,
} as const;
export type Pricing12IconToken = keyof typeof pricing12Icons;
export const pricing12Assurances: Array<{ iconToken: Pricing12IconToken; label: string }> = [
  { iconToken: "shield", label: "SOC 2 Type II certified" },
  { iconToken: "refresh", label: "14-day money-back guarantee" },
  { iconToken: "card", label: "No card required to browse" },
];
...
export const pricing12ContentDefaults = {
  heading: "One plan. The whole studio.",
  description:
    "Every component, block, and template we ship, plus direct access to the team that builds them. No tiers to outgrow.",
};

export default function AdaptedPricing12({
  features = pricing12Features,
  assurances = pricing12Assurances,
  heading = pricing12ContentDefaults.heading,
  description = pricing12ContentDefaults.description,
}: {
  features?: Array<{ text: string }>;
  assurances?: Array<{ iconToken: Pricing12IconToken; label: string }>;
  heading?: string;
  description?: string;
}) {
  ...
  return (
    <section className="w-full bg-white dark:bg-neutral-950 py-16 sm:py-20 lg:py-24 px-4 sm:px-6 lg:px-8">
      ...
      >
        ...
        >
          {assurances.map(({ iconToken, label }) => {
            const Icon = resolveIconToken(iconToken, pricing12Icons);
            return (
            <span
              key={label}
