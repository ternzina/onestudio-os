...
export type Navbar4IconToken = keyof typeof navbar4Icons;
export type Navbar4Section = {
  label: string;
  items: Array<{ iconToken: Navbar4IconToken; label: string; hint: string }>;
};

export const navbar4Sections: Navbar4Section[] = [
  { label: "Product", items: [
    {
      iconToken: "layers",
      label: "Platform",
  ...
  { label: "Company", items: [
    { iconToken: "users", label: "About", hint: "Who we are and what we build" },
    {
      iconToken: "newspaper",
      label: "Blog",
    ...
    { iconToken: "support", label: "Support", hint: "Reach a human within one hour" },
  ] },
];

const FLAT = ["Pricing", "Changelog"];

export default function AdaptedNavbar4({
  sections = navbar4Sections,
}: {
  sections?: Navbar4Section[];
}) {
              ...
              <li key={key} className="relative" data-menu-root>
                ...
                <AnimatePresence>
                  {menu === key && (
                    ...
                    >
                      {items.map((l) => {
                        const Icon = resolveIconToken(l.iconToken, navbar4Icons);
                        return (
                          <a
                            key={l.label}
                            href="#"
