...
export type AdaptedNavigation9Props = {
  links: MenuLinkItem[];
};

export const navigation9Links: MenuLinkItem[] = [
  { label: "Products", href: "#" },
  { label: "Platform", href: "#" },
  { label: "Pricing", href: "#" },
  { label: "Company", href: "#" },
...
export default function AdaptedNavigation9({ links }: AdaptedNavigation9Props) {
  ...
  return (
    <section className="relative w-full bg-white dark:bg-neutral-950">
      ...
      <div className="w-full pt-4">
          ...
          <div className="mx-auto w-full max-w-[1400px]">
            ...
            >
              ...
              <div className="hidden md:flex items-center gap-8 px-4">
                {links.map((link, index) => (
                  <a key={`${link.label}-${link.href}-${index}`} href={link.href ?? "#"} className="text-sm text-neutral-600 dark:text-neutral-300 hover:text-neutral-900 dark:hover:text-white transition-colors">
                    {link.label}
                  </a>
                ))}
              </div>
              ...
              <div className="flex items-center gap-1.5">
                <button className="hidden sm:inline-flex items-center px-5 py-2.5 rounded-[10px] bg-neutral-900 dark:bg-white text-white dark:text-neutral-900 text-sm font-medium hover:opacity-90 transition-opacity cursor-pointer">Get Started</button>
                <button
            ...
            <AnimatePresence>
              {open && (
                ...
                >
                  {links.map((link, index) => (
                    <a key={`${link.label}-${link.href}-${index}`} href={link.href ?? "#"} className="text-sm text-neutral-800 dark:text-neutral-200 py-2">
                      {link.label}
                    </a>
                  ))}
                  <button className="mt-2 w-full px-4 py-2.5 rounded-[10px] bg-neutral-900 dark:bg-white text-white dark:text-neutral-900 text-sm font-medium cursor-pointer">Get Started</button>
                </motion.div>
              )}
            </AnimatePresence>
          </div>
        ...
        <div aria-hidden="true" className="h-[var(--rb-section-min-h,100vh)] overflow-hidden px-4 sm:px-6 lg:px-8">
