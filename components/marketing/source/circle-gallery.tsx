...
export interface CircleGalleryProps {
  ...
  /** Radius of the circle as a percentage of its bounded stage (10-50) */
...
export const CircleGallery: React.FC<CircleGalleryProps> = ({
  ...
  const getStageDimension = useCallback(() => {
    const bounds = containerRef.current?.getBoundingClientRect();
    if (!bounds || bounds.width <= 0 || bounds.height <= 0) {
      return Math.min(window.innerWidth, window.innerHeight);
    }

    // Keep the circle sized to its actual showcase frame. The viewport cap
    // preserves the accepted desktop scale while preventing ultrawide screens
    // from turning a bounded visual into a giant clipped effect.
    return Math.min(
      window.innerWidth,
      window.innerHeight,
      bounds.width,
  ...
  const calculateCirclePositions = useCallback(
    (animated = false) => {
      ...
      const viewportDimension = getStageDimension();
    ...
    [getStageDimension, totalItems, radiusPercent],
  );
  ...
  }, [animationDuration, getStageDimension, itemScale, radiusPercent, setupDraggable, totalItems]);
